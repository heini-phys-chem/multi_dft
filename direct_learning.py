...
def do_LC(X_train, Q_train, X_test, Q_test, Y_PBE, Y_PBE_PBE0, Y_PBE0_PBE86, Y_PBE86_wPBE, Y_wPBE, Y_wPBE_train):
  ...
  N_PBE86 = [   8,  16,  32,   64 ]
  N_wPBE  = [   2,   4,   8,   16 ]
  N_wPBE  = [   2,   4,   8,   16, 32, 64, 128, 256, 512, 1024 ]
...
#          Yp_PBE0_PBE86 = do_ML(X_train[ti], X_test, Y_PBE0_PBE86[ti], sigma_PBE86, Q_train[ti], Q_test)
  ...
  #        sigma_wPBE = 1.
          ...
          Y_multi = do_ML(X_train[ti], X_test, Y_wPBE_train[ti], sigma_wPBE, Q_train[ti], Q_test)
...
#          Y_multi  = Yp_PBE + Yp_PBE_PBE0 + Yp_PBE0_PBE86 + Yp_PBE86_wPBE
          ...
          sp.write("> N = {:>5}, MAE: {:>4,.4f} [✔]".format(N_wPBE[train], mae_multi))
#        print("N: {:.2f},\tMAE: {:.2f}".format(N_wPBE[train], mae_multi))
        sp.ok("✔")

        e_multi   = np.append(e_multi, MAE_multi)

  e_multi   = e_multi.reshape(nModels,len(N_wPBE)).mean(axis=0)

  print("\nN_wPBE,N_PBE86,N_PBE0,N_PBE,e_direct")
  for i in range(len(e_multi)):
      print("{},{},{},{},{:.4f}".format(N_wPBE[i], 1, 1, 1, e_multi[i]))
...
def main():
  ...
  Y_wPBE_train = Y_wPBE[train]
  Y_wPBE       = Y_wPBE[test]


  start = time()
  isDone = do_LC(X_train, Q_train, X_test, Q_test, Y_PBE, Y_PBE_PBE0, Y_PBE0_PBE86, Y_PBE86_wPBE, Y_wPBE, Y_wPBE_train)
  end = time()
